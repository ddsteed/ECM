...
c----------------------------------------------------------------
       call expectwf(kw,nr,nu,ng,nbi,nwv,h,rmin,eigv,u,
     c rmesh,wf,expr)
c----------------------------------------------------------------
...
C--- 
      write(0,320)
      write(6,350) rmin,rmax,rstep,rmatch,iel,ktyp,
     #nvp,ng,nbmax,nbi,nwv,nci-1,ncf-1
...
C---   for eigenvalue convergence
      do 10 i=1,nbmax
        read(5,*) kstd(i),kcon(i),qeng(i),vtol(i),dev(i),
     #             ul0(i),ur0(i)
        write(6,410) i-1,kstd(i),kcon(i),qeng(i),vtol(i),dev(i),ul0(i),ur0(i)
10    continue
C--- 
	 h=rstep
        nr=int(rmax-rmin)/h
...
C---
          ...
          De1 = De0*beta
C---
          write(6,414) we,we*aucm,wexe,wexe*aucm,weye,weye*aucm,
     # weze,weze*aucm,wete,wete*aucm,wese,wese*aucm,Nturn
C---
          if (Add .gt. 0.0) then
...
c----------------------------------------------------------------
 10    call solvese(potf, Q, ktyp, iel, rmu, rmin, rmatch,
...
c
      ...
      common /vmon/ rv(20000),vnum(20000)
      data one,two,three,four,ten,twelve/1.0d0,2.0d0,3.0d0,
     #4.0d0,10.0d0,12.0d0/
c---------------------------------------------------------------------
...
c---------------------------------------------------------------------
      Tnumerov(r) =  h*h/twelve * (two*rmu * (V(r) - Q) +
     #              (iel*(iel+1))/(r*r) )
c---------------------------------------------------------------------
...
c---------------------------------------------------------------------
      ...
      elseif (ktyp .eq. 9) then
        do i = 1, nrmesh
             r = h*(i-1) + rmin
          T(i) = h*h/twelve * (two*rmu * (vnum(i) - Q) +
     #             (iel*(iel+1))/(r*r) )
        enddo
      endif
c---------------------------------------------------------------------
...
c  same for uR
        ...
        uR(2) = uR2
      do i = 3, mR+2
        uR(i) = ( (two + ten*T(nrmesh-i+2))*uR(i-1) -
     &            ( one - T(nrmesh-i+3))*uR(i-2) ) /
     &            ( one - T(nrmesh-i+1) )
      enddo
c---------------------------------------------------------------------
c  scale uR
...
c---------------------------------------------------------------------
      ...
      uLprime = 16.0d0/(21.0d0 * h)  * 
     &          (-A1L + 37.0d0*A2L/32.0d0 - 
     &            37.0d0*B1L/5.0d0 - 17.0d0*B2L/40.0d0 )
      uRprime = 16.0d0/(21.0d0 * h)  *
     &          (-A1R + 37.0d0*A2R/32.0d0 -
